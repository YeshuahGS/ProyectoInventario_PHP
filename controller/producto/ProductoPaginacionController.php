...
$buscadorCount = ProductoModelo::CountBuscadorProductos($data);
...
$buscador = ProductoModelo::BuscadorProductos($data, $desde, $por_pagina);
$totalProductos = ProductoModelo::BuscadorProductosTotales($data);
foreach ($totalProductos as $key => $value) {
    $totalPrecioCompra += $value['precio_compra'];
    $totalPrecioVenta += $value['precio_venta'];
    $stock += $value['stock'];
}
if ($_SESSION['busquedaProducto'] != "") :
    if (COUNT($buscador) > 0) :
